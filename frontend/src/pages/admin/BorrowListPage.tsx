...
const BorrowListPage = () => {
  ...
  const fetchBorrows = async () => {
    try {
      ...
      setBorrows(
        data.sort(
          (a: BorrowRecord, b: BorrowRecord) => Number(b.id) - Number(a.id),
        ),
      );
    } catch (err) {
  ...
  const paginatedGroups = sessionGroups.slice(
    (page - 1) * PAGE_SIZE,
    page * PAGE_SIZE,
  );
  ...
  const handleReturn = async () => {
    ...
    setIsReturning(true);
    try {
      const searchRes = await borrowRecordApi.search({
        title: selectedBorrow.bookTitle,
      });
      const records = searchRes.data ?? [];
      const match = records.find(
        (r) => r.borrowRecordId === Number(selectedBorrow.id),
      );

      if (!match)
        throw new Error("Không tìm thấy mã barcode khớp với phiếu mượn này.");
      ...
      toast({
        ...
        description: res.data.hasFinePending
          ? `Phạt: ${res.data.fineAmount.toLocaleString("vi-VN")}đ`
          : "Thành công!",
      });
  ...
  const handleReturnAll = async (group: SessionGroup) => {
    const activeRecords = group.records.filter(
    ...
    try {
      ...
      toast({
        ...
        description: `Đã xử lý xong ${activeRecords.length} cuốn cho ${group.userName}.`,
      });

      await fetchBorrows();
    } catch (err: any) {
      toast({
        title: "Lỗi khi trả hàng loạt",
        description:
          err.response?.data?.message || err.message || "Đã xảy ra lỗi.",
        variant: "destructive",
      });
  ...
  const fine = selectedBorrow
    ? calculateFine(
        String(selectedBorrow.dueDate ?? "").slice(0, 10),
  ...
  const getSessionStatusSummary = (records: BorrowRecord[]) => {
    ...
    const activeCnt = records.filter(
      (r) => r.status === "borrowed" || r.status === "overdue",
    ).length;
    if (allReturned)
      return (
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <span className="text-sm text-muted-foreground ml-auto">
          {sessionGroups.length} phiếu mượn
        </span>
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="glass-card flex flex-col items-center justify-center py-16 gap-2 text-destructive">
            ...
            <Button variant="outline" size="sm" onClick={fetchBorrows}>
              Thử lại
            </Button>
          </div>
          ...
          paginatedGroups.map((group) => {
            ...
            const hasActive = group.records.some(
              (r) => r.status === "borrowed" || r.status === "overdue",
            );
            return (
